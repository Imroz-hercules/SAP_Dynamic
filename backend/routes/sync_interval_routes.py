...
"""
Sync interval management routes with role-based access control - Simple Date/Time based version
No next_sync calculations - sync only happens at specified date/time
"""
...
def update_sync_setting(sync_type):
    """Update sync interval setting (no auth required)"""
    try:
        ...
        if sync_time is None and sync_date is None and is_enabled is None and sync_interval_minutes is None:
            return jsonify({'error': 'At least one field (sync_time, sync_date, is_enabled, or sync_interval_minutes) is required'}), 400
        ...
        with PostgresSessionLocal() as db:
            ...
            if sync_interval_minutes is not None:
                ...
                # When saving an interval for process_orders, ensure sync is enabled so the scheduler runs
                if sync_type == 'process_orders' and sync_interval_minutes >= 1:
                    setting.is_enabled = True
            
            setting.updated_at = get_utc_now()
            setting.updated_by = getattr(request, 'current_user', {}).get('user_id', None)
            ...
            try:
                log_entry = SystemLog(
                    ...
                    details=json.dumps({
                        ...
                        "is_enabled": setting.is_enabled,
                        "sync_interval_minutes": setting.sync_interval_minutes,
                        "updated_by": (getattr(request, 'current_user', None) or {}).get('username', 'unknown')
                    }),
                    category="sync",
...
def save_sync_setting(sync_type):
    """Save sync setting and refresh scheduler (no auth required)"""
    try:
        data = request.get_json()
        sync_time = data.get('sync_time')
        sync_date = data.get('sync_date')
    ...
    try:
        ...
        with PostgresSessionLocal() as db:
            ...
            setting = SyncIntervalSettings(
                ...
                created_by=(getattr(request, 'current_user', None) or {}).get('user_id'),
                updated_by=(getattr(request, 'current_user', None) or {}).get('user_id')
            )
