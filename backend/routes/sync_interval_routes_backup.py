...
"""
Sync interval management routes with role-based access control
"""
...
def update_sync_setting(sync_type):
    ...
    try:
        ...
        if sync_time is None and is_enabled is None:
            return jsonify({'error': 'At least one field (sync_time or is_enabled) is required'}), 400
        ...
        with PostgresSessionLocal() as db:
            ...
            if interval_minutes is not None:
                ...
                # Update next_sync based on new interval
                if setting.is_enabled and setting.last_sync:
                    setting.next_sync = calculate_next_sync_time(setting.last_sync, interval_minutes)
            
            if is_enabled is not None:
                setting.is_enabled = is_enabled
                if is_enabled and setting.last_sync:
                    # If enabling, set next sync
                    setting.next_sync = calculate_next_sync_time(setting.last_sync, setting.interval_minutes)
                elif not is_enabled:
            ...
            return jsonify({
                ...
                'setting': {
                    ...
                    'sync_type': setting.sync_type,
                    'interval_minutes': setting.interval_minutes,
                    'interval_hours': round(setting.interval_minutes / 60, 2),
                    'is_enabled': setting.is_enabled,
                    'last_sync': format_datetime_for_api(setting.last_sync),
                    'next_sync': format_datetime_for_api(setting.next_sync),
...
def save_sync_setting(sync_type):
    ...
    try:
        ...
        if interval_minutes is None and is_enabled is None:
            return jsonify({'error': 'At least one field (interval_minutes or is_enabled) is required'}), 400
        
        if interval_minutes is not None and (not isinstance(interval_minutes, int) or interval_minutes < 1):
...
def create_sync_setting():
    ...
    try:
        ...
        if not sync_type or not interval_minutes:
            return jsonify({'error': 'sync_type and interval_minutes are required'}), 400
