...
/**
 * Engineering — Workstream A, task A8.
 *
 * One place for the plant connection configuration that previously lived only
 * in `.env` on the server. Everything here resolves through
 * services/runtime_config.py: database -> .env -> documented default.
 *
 * The `source` badge on each field is the useful part. It says whether a value
 * is stored, inherited from `.env`, or just the built-in default — which is
 * otherwise invisible, and the usual reason a change "does not take".
 *
 * Admin only, and it uses apiFetch so the bearer token is attached.
 */
...
const Engineering: React.FC = () => {
  ...
  return (
    <WaterSystemLayout title="Engineering" subtitle="Plant connection configuration">
      <div className="space-y-6 w-full px-4 py-6">
        {notice && (
          <div
            className={`fixed top-4 right-4 z-50 px-3 py-2 rounded-md shadow-lg text-sm max-w-md ${
              notice.kind === 'success'
                ? light
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            <h2 className={`text-xl font-bold ${light ? 'text-slate-700' : 'text-cyan-400'}`}>
              Engineering
            </h2>
            <p className={`text-xs mt-1 max-w-2xl ${light ? 'text-slate-500' : 'text-slate-400'}`}>
              Connection settings for SAP and SQL Server. Saved values take effect on
              the next call — no restart. Anything not saved here falls back to
              <span className="font-mono"> .env</span> on the server, and then to a
              built-in default; the badge on each field says which applies.
            </p>
          </div>
        ...
        {missing.length > 0 && (
          ...
          >
            ...
            <div className={`text-xs ${light ? 'text-red-700' : 'text-red-300'}`}>
              ...
              SAP calls will fail until these are set here or in{' '}
        ...
        {excluded.length > 0 && (
          <div className={`rounded-xl p-4 ${panel}`}>
            ...
            <p className={`text-[11px] mb-3 ${light ? 'text-slate-500' : 'text-slate-400'}`}>
              These cannot work as a live setting. A control for them would appear to
              save and then change nothing, which is worse than not offering it.
            </p>
