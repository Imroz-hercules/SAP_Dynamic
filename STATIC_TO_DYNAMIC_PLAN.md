...
| **A8** ✅ | **Engineering page — done 20 Aug 2026.** `/engineering` behind `AuthGuard` + `AdminGuard`, with an admin-gated Sidebar entry. `services/runtime_config.py` is the single reader (DB → `.env` → default) and **all six** SAP config read sites go through it — four of which read at *import*, which is why this was a developer task. Each field shows whether its value came from the database, `.env`, or a built-in default. Secrets masked; a masked value sent back counts as unchanged. Poll intervals are **read-only** — APScheduler bakes them in and `app_scheduler.py` is B's file (B5). Admin's dead SAP block is **marked, not deleted**, because removing ~100 lines would shift the line numbers B6 cites. **⚠️ Two things that matter to B:** the committed SAP credential literals are now gone from the Python source (most of B5's code half), and **`app.py` gained one blueprint registration** — re-check any `app.py` line number before starting. | ✅ Verified in a browser and through the API: changed a value, saved, badge flipped ENV → DATABASE, and the next outbound call used it — no restart. Test SAP returned HTTP 200 in 175 ms. **Also fixes the mock-mode gap:** the order pull now honours mock mode, where during A1 it hit the real SAP host. |
...
> **Status 21 Aug 2026:** Core B is **done** (B1–B6, B8; B7/B9 mostly done). Live checklist and leftovers: [`WORKSTREAM_B_STATUS.md`](WORKSTREAM_B_STATUS.md).
...
| **B9** | **Live monitoring screens.** `LiveMonitor.tsx` (23 hardcoded tags) and `LiveDataTable.tsx` (16) list tags inline. Drive from `scadaConfigApi`. `ScadaContext.tsx` also hardcodes the business-name mapping (`cleaningScale`, `dryWheatScale`, …) mirroring `scada_routes.py:768–782`; move that mapping into the registry's `display_name`. | Adding a tag to the registry makes it appear on the live screens. | ⚠️ Live screens done; display_name polish open |
